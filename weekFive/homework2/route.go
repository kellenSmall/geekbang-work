...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	...
	for _, s := range segs {
		var dls []Middleware
		if len(ns) == 1 {
			ns, dls = ns[0].childOfMdls(s)
			if dls != nil && len(dls) > 0 {
				m = append(m, dls...)
			}
		} else {
			for _, n := range ns {
				nss, dls := n.childOfMdls(s)
				if dls != nil && len(dls) > 0 {
					m = append(m, dls...)
				}
				ns = append(ns, nss...)
...
func (n *node) childOfMdls(path string) ([]*node, []Middleware) {
	m := make([]Middleware, 0)
	ns := make([]*node, 0)
	res, ok := n.children[path]
	if !ok {
		if n.paramChild != nil {
			return append(ns, n.paramChild), append(m, n.paramChild.mdls...)
		}
		if n.starChild != nil {
			return append(ns, n.starChild), append(m, n.starChild.mdls...)
		}
	}
	if n.paramChild != nil {
		return append(ns, n.paramChild), append(m, n.paramChild.mdls...)
	...
	if n.starChild != nil {
		if n.starChild.mdls != nil {
			m = append(m, n.starChild.mdls...)
		} else {
			ns = append(ns, n.starChild)
		}
	}
	if res.mdls != nil {
		m = append(m, res.mdls...)
	}
	return append(ns, res), m
}
