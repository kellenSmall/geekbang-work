...
func isRegexpNode(val string) bool {
...
func findRegexp(val string) string {
	matched, err := regexp.Compile(`\((.*)\)`)
	if err != nil {
		return ""
	}
	submatch := matched.FindStringSubmatch(val)
	if len(submatch) < 2 {
		return ""
	}
	return submatch[1]
...
func (n *node) childOrCreate(path string) *node {
	...
	// 以 : 开头，我们认为是参数路由
	if path[0] == ':' {
		if n.starChild != nil && isRegexpNode(path) {
		...
		if n.paramChild != nil {
			...
			if isRegexpNode(path) {
				...
				regstr := findRegexp(path)
				if regstr == "" {
					regstr = "*"
				}
				regex := regexp.MustCompile(regstr)
				index := strings.Index(path, "(")
				n.regChild = &node{path: path, regExpr: regex, paramName: path[1:index], typ: nodeTypeReg}
				return n.regChild
			}
